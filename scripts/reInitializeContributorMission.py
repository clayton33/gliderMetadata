...
for d in df.itertuples():
    ...
    for i in roleAbbrev:
        ...
        if(i == 'PI'):
            crQ = models.ContributorRole.objects.get(contributor_role='PI')
            contributorPeopleRolePk.append(crQ.pk)
        if(i == 'Operator'):
            crQ = models.ContributorRole.objects.get(contributor_role='Operator')
            contributorPeopleRolePk.append(crQ.pk)
        # append mission pk
        if missionQ is None:
...
for row in contributorDf.itertuples():
    icm = models.ContributorMission.objects.get(contributor_mission = models.Mission.objects.get(pk=getattr(row, 'contributorMissionPk')),
                                                contributor_missionPerson = models.ContributorPeople.objects.get(pk=getattr(row, 'contributorPersonPk')))
    icm.contributor_missionRole = models.ContributorRole.objects.get(pk=getattr(row, 'contributorPersonRolePk'))
    icm.save()